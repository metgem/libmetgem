...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef float[:] compare_spectra_to_reference_nogil(vector[double] mzvec, vector[peak_t *] datavec, vector[int] data_sizes,
                                                 double mzref, peak_t *dataref, int refsize,
                                                 double mz_tolerance, int min_matched_peaks, object callback=None):
    cdef:
        int i
        int size = mzvec.size()
        float[:] result = cvarray(shape=(size,), itemsize=sizeof(float), format='f')
    ...
    with nogil:
        for i in range(size):
            if data_sizes[i] == 0:
                ...
                result[i] = cosine_score_nogil(mzvec[i], datavec[i], data_sizes[i],
                                               mzref, dataref, refsize,
                                               mz_tolerance, min_matched_peaks)
...
def compare_spectra_to_reference(vector[double] mzvec, list datavec, double mzref, np.ndarray[np.float32_t, ndim=2] dataref, double mz_tolerance, int min_matched_peaks, object callback=None):
    cdef:
        np.ndarray[np.float32_t, ndim=2] tmp_array
        int size = mzvec.size()
        int i
    ...
    result = np.asarray(compare_spectra_to_reference_nogil(mzvec, data_p, data_sizes,
                                                           mzref, np_arr_pointer(dataref), dataref.shape[0],
                                                           mz_tolerance, min_matched_peaks, callback))
    result[result>1] = 1
    return result